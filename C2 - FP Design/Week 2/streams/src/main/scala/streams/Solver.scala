...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(
    ...
    b.legalNeighbors.to(LazyList) map { case (neigh, move) =>
      (neigh, move :: history)
    }
  }
  ...
  def from(
    ...
    case (block, moves) #:: tail =>
      val newExplore = explored + block
      val successors = from(
        tail ++ newNeighborsOnly(
          neighborsWithHistory(block, moves),
          newExplore
        ),
        newExplore
      )
      if (done(block)) (block, moves) #:: successors else successors
  }

  /** The lazy list of all paths that begin at the starting block.
    */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from((startBlock, Nil) #:: LazyList.empty, Set.empty)
  ...
  /** The (or one of the) shortest sequence(s) of moves to reach the
    ...
    * position.
    */
  lazy val solution: List[Move] = pathsFromStart match {
    case LazyList()       => Nil
