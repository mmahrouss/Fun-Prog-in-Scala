...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromFunction {
      case Json.Num(value) =>
        if (value.isValidInt) Option(value.toInt) else Option.empty
      case _ => Option.empty
    }

  /** A decoder for `String` values */
  implicit val strDecoder: Decoder[String] =
    Decoder.fromPartialFunction { case Json.Str(value) =>
      value
    }

  /** A decoder for `Boolean` values */
  implicit val boolDecoder: Decoder[Boolean] =
    Decoder.fromPartialFunction { case Json.Bool(value) =>
      value
    }
  ...
  implicit def listDecoder[A](implicit
    ...
    def decodeList(ls: List[Json]): Option[List[A]] = ls match {
      case x :: xs =>
        (decoder.decode(x), decodeList(xs)) match {
    ...
    Decoder.fromFunction {
      case Arr(items) => decodeList(items)
      case _          => Option.empty
    }
  }
